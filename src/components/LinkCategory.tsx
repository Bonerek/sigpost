...
const SortableLink = ({ link, onEdit, onDelete, editMode }: SortableLinkProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative group/link">
      ...
      <div className="flex items-center gap-3 pl-3 pr-6 py-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors duration-200">
        <Tooltip>
    ...
    <Card className="overflow-hidden border-0 shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      <div className="p-6 space-y-1">
        {editMode ? (
          ...
          links.map((link) => (
            <div key={link.id} className="flex items-center gap-3 pl-3 pr-6 py-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors duration-200">
              <Tooltip>
                <TooltipTrigger asChild>
                  <a
                    href={link.url}
